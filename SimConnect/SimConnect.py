...
class SimConnect:
	...
	def handle_id_event(self, event):
		...
		if uEventID == self.dll.EventID.EVENT_SIM_START.value:
			LOGGER.info("SIM START")
			self.running = True
		if uEventID == self.dll.EventID.EVENT_SIM_STOP.value:
			LOGGER.info("SIM Stop")
			self.running = False
		if uEventID == self.dll.EventID.EVENT_SIM_PAUSED.value:
			LOGGER.info("SIM Paused")
			self.paused = True
		if uEventID == self.dll.EventID.EVENT_SIM_UNPAUSED.value:
			LOGGER.info("SIM Unpaused")
			self.paused = False
		if uEventID == self.dll.EventID.EVENT_SIM_PAUSE_EX1.value:
			LOGGER.info("SIM Pause Ex1: %d", event.dwData)
			self.paused = event.dwData != 0
		...
		try:
			...
			if self.IsHR(err, 0):
				...
				self.dll.SubscribeToSystemEvent(
					self.hSimConnect, self.dll.EventID.EVENT_SIM_START.value, b"SimStart"
				)
				# The user is navigating the UI.
				self.dll.SubscribeToSystemEvent(
					self.hSimConnect, self.dll.EventID.EVENT_SIM_STOP.value, b"SimStop"
				)
				# Request a notification when the flight is paused
				self.dll.SubscribeToSystemEvent(
					self.hSimConnect, self.dll.EventID.EVENT_SIM_PAUSED.value, b"Paused"
				)
				# Request a notification when the flight is un-paused.
				self.dll.SubscribeToSystemEvent(
					self.hSimConnect, self.dll.EventID.EVENT_SIM_UNPAUSED.value, b"Unpaused"
				)
				# Request a notification when the pause state changes
				self.dll.SubscribeToSystemEvent(
					self.hSimConnect, self.dll.EventID.EVENT_SIM_PAUSE_EX1.value, b"Pause_EX1"
				)
	...
	def map_to_sim_event(self, name):
		...
		self.dll.EventID = Enum(self.dll.EventID.__name__, names, start=0)
